...
show(io::IO,x::ASCIIChar) = print(io,char(x))

## Bits representation ##

bits(x::ASCIIChar) = bin(reinterpret(Uint8,x),8)
...
convert{T<:ASCIIChar}(::Type{Vector{T}}, x::Matrix{T}) = reshape(x,length(x))
convert{T<:ASCIIChar}(::Type{Matrix{T}}, x::Vector{T}) = reshape(x,1,length(x))
